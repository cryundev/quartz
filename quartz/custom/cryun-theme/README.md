...
- `src/CryunExplorer.tsx`: 목차 아래 버튼과 네이티브 `dialog`, Quartz의 `ctx.trie`를 읽는 접기/펼치기 탐색기를 렌더링한다. 공유 트리는 변경하지 않는다.
...
`quartz.config.yaml`에서 문서 트리를 우측 슬롯의 30번 위치에 둔다. 우측에는 그래프(10) → 목차(20) → 문서 트리 버튼(30)을 표시한다. 프레임은 그 아래에 `#quartz-body`로 이동하는 “맨 위로” 링크를 배치한다. 별도 스크롤 이벤트나 문서 데이터 변경 없이 기본 앵커 탐색을 사용한다. 백링크는 `afterBody` 슬롯에서 본문 아래에 표시한다. 백링크가 없으면 기본 플러그인 동작에 따라 영역도 숨겨진다.

- 데스크톱에서는 그래프와 목차를 담은 우측 카드 전체가 화면 상단 24px에 붙어 스크롤을 따라온다. 좌우 20px 패딩을 유지하며, 높이를 제한하거나 별도로 스크롤하지 않는다. 카드가 화면보다 길면 페이지와 함께 내려가 아래쪽 목차를 보여준 뒤 화면 하단 24px에 맞춰 붙는다. 카드 높이는 `ResizeObserver`로 측정하여 목차 접기·글꼴·창 크기 변경에 대응하고, SPA 이동 때 관찰을 해제한다. 작은 그래프의 `localGraph.zoom: false` 설정은 유지한다.
