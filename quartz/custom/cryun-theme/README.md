...
`quartz.config.yaml`에서 문서 탐색을 `toolbar` 그룹의 검색 다음 위치(25)에 둔다. 우측에는 그래프(10) → 목차(20)만 표시한다. 백링크는 `afterBody` 슬롯에서 본문 아래에 표시한다. 백링크가 없으면 기본 플러그인 동작에 따라 영역도 숨겨진다.
...
- 문서 탐색 팝업에서만 목록 하나가 스크롤된다. 팝업이 열린 동안 배경 스크롤은 잠긴다. 닫기 버튼·Esc·배경 클릭으로 닫히며, 일반 링크 클릭과 SPA 교체 때 모달 상태를 해제한다. 보조키를 사용한 새 탭 열기는 브라우저 기본 동작을 유지한다.
