...
- `src/explorer.inline.ts`: 팝업 열기·닫기와 트리 초기화를 관리한다. 포커스 제한은 브라우저의 모달 동작을 사용하며, 검색창에서도 Esc로 닫힌다. 한글 등 IME 조합 중 Esc는 입력기에 맡긴다. SPA 이동 때 모달 상태와 이벤트를 해제한다.
- `src/explorer-tree.inline.ts`: 공개된 트리 DOM을 문서명·상위 폴더 경로로 색인하고 입력 즉시 필터링한다. 검색 중 펼침은 임시 상태이며, 검색 해제 시 기존 폴더 상태를 복원한다. 폴더 저장과 검색 이벤트도 Quartz 정리 수명주기를 따른다.
...
- 문서 트리 팝업 상단 검색창은 문서명·폴더 경로의 부분 일치로 필터링한다. 대소문자와 유니코드 정규화 차이를 무시하며, 여러 단어는 모두 일치해야 한다. 일치 항목의 상위 폴더를 펼치고 일치 개수·결과 없음 안내·검색어 지우기를 제공한다. 같은 페이지에서 팝업을 다시 열면 검색어를 유지하고, 다른 문서로 이동하면 검색어를 비워 새 트리를 표시한다. 검색은 렌더링된 공개 트리만 사용하며, 본문 검색은 기존 사이트 검색에서 제공한다.
- 문서 트리 팝업에서만 목록 하나가 스크롤된다. 검색창은 목록 위에 고정되고 검색 중에도 팝업 높이를 유지한다. 팝업이 열린 동안 배경 스크롤은 잠긴다. 닫기 버튼·Esc·배경 클릭으로 닫히며, 일반 링크 클릭과 SPA 교체 때 모달 상태를 해제한다. 보조키를 사용한 새 탭 열기는 브라우저 기본 동작을 유지한다.
