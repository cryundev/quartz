...
- 데스크톱: `.cryun-context`만 높이를 제한하고 스크롤한다. 목차·탐색기·백링크 내부에는 별도 스크롤이나 목록 끝 마스크를 적용하지 않는다. 패널 끝에서는 페이지로 자연스럽게 스크롤이 이어진다. 작은 그래프는 공식 `localGraph.zoom: false` 옵션으로 휠을 가로채지 않게 하고, 전체 그래프 확대·축소는 유지한다.
