...
- `src/CryunExplorer.tsx`: 목차 아래 별도 카드의 문서 트리·맨 위로 버튼과 네이티브 `dialog`, Quartz의 `ctx.trie`를 읽는 접기/펼치기 탐색기를 렌더링한다. 공유 트리는 변경하지 않는다.
- `src/explorer.inline.ts`: 팝업 열기·닫기와 폴더 펼침 상태를 관리한다. Esc·포커스 제한은 브라우저의 모달 동작을 사용하고, SPA 이동 때 모달 상태와 이벤트를 해제한다.
- `src/theme.inline.ts`: 좁은 화면의 목차 접기, 우측 카드 높이 측정, 팝업 초기화, 활성 메뉴 위치. Quartz의 `nav`/`addCleanup` 수명주기를 따른다.
...
`quartz.config.yaml`에서 문서 트리를 우측 슬롯의 30번 위치에 둔다. 우측에는 그래프(10) → 목차(20) → 문서 트리 버튼(30)을 표시한다. 그래프와 목차는 공식 `reading-context` 그룹으로 한 카드에 묶는다. 문서 트리 컴포넌트는 별도 카드 안에 팝업 버튼과 `#quartz-body`로 이동하는 “맨 위로” 링크를 배치한다. 두 카드 사이 간격은 16px이며, 표시할 그래프·목차가 없으면 빈 카드를 숨긴다. 별도 스크롤 이벤트나 문서 데이터 변경 없이 기본 앵커 탐색을 사용한다. 백링크는 `afterBody` 슬롯에서 본문 아래에 표시한다. 백링크가 없으면 기본 플러그인 동작에 따라 영역도 숨겨진다.

- 데스크톱에서는 그래프·목차 카드와 탐색 버튼 카드가 함께 화면 상단 24px에 붙어 스크롤을 따라온다. 각 카드의 좌우 20px 패딩을 유지하며, 높이를 제한하거나 별도로 스크롤하지 않는다. 카드가 화면보다 길면 페이지와 함께 내려가 아래쪽 목차를 보여준 뒤 화면 하단 24px에 맞춰 붙는다. 카드 높이는 `ResizeObserver`로 측정하여 목차 접기·글꼴·창 크기 변경에 대응하고, SPA 이동 때 관찰을 해제한다. 작은 그래프의 `localGraph.zoom: false` 설정은 유지한다.
- 모바일에서는 그래프를 숨기고 “목차 · 문서 트리” 영역을 펼쳐 목차와 버튼을 사용할 수 있다. 버튼은 모바일에서도 텍스트를 표시한다. 목차가 없는 폴더·태그 페이지에도 문서 트리 버튼을 제공한다.
